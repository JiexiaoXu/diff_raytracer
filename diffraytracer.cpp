...
// vec3const refract(const vec3const &I, const vec3const &N, const double eta_t, const double eta_i=1.f) { // Snell's law
//     double cosi = - std::max(-1.0, std::min(1.0, I.dot(N)));
//     if (cosi<0) return refract(I, -N, eta_i, eta_t); // if the ray comes from the inside the object, swap the air and the media
//     double eta = eta_i / eta_t;
//     double k = 1 - eta*eta*(1 - cosi*cosi);
//     return k<0 ? vec3const{1,0,0} : I*eta + N*(eta*cosi - std::sqrt(k)); // k<0 = total reflection, no ray to refract. I refract it anyways, this has no physical meaning
// }
